...
7. 剔除选中方向的异常簇，对剩余数据重新聚类（基线随异常剔除收紧，检出更多边缘异常），循环直到无新增异常
...
**慢通信（带宽聚类）**：解析完成后先做全局回填 —— 重新扫描 `.db`、重建并行拓扑，将各 rank 的集合通信 op 按 (opType, count) 对齐（跳过 pp / Send / Recv 点对点），用组内最短时长计算每类 op 的带宽，写入 CSV 动态列 `{domain}_<opType>_<count>`。检测时对每个非 PP/非 embd 域，**先按算子类型（opType）分类**：每个通信组在某一 opType 下取 **count 最大**的带宽作代表（大数据量更能体现真实带宽）；再取所有组代表 count 的最大值并求其**量级**（个/十/百/千/万…，即 `10^⌊log10⌋`），剔除代表 count 小于该量级的组（该组不参与该算子检测）；对剩余组的代表带宽用共享 kmeans 递归聚类（`clustering.Detect`，方向 min：带宽越小越慢，阈值 `SlowCommRatio` 默认 1.3）检出劣化组与劣化指数（= 基线带宽/该组带宽，>1）。最后将该域**所有 opType** 判出的劣化指数放在一起取最大值，上报对应组为该域的唯一通信异常，写入 `comm`（复用 `comm_domain_result`）。参与聚类数据 < 2 或该域无带宽列 → 跳过。
...
7. 递归进入每个异常簇（深度 ≤ 10）减少误检：更深层异常替换父簇，更深层静默则保留父簇成员
8. 异常卡的劣化值 = 卡值 / 第一次 kmeans（全部数据）的基线簇均值（统一基准）
