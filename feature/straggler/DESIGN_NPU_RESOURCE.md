...
2. 基线簇 = 成员最多的簇；成员数并列 → 方向极值簇
   （DirHigh→均值最低，DirLow→均值最高）
3. 对每个非基线簇 c（单侧判定，只查异常方向）：
   z_c = |mean(c) − mean(基线)| / scale[m]
   scale[m] = 各卡历史噪声 1.4826×baseline.Mad 的中位数（自我标定）
   if z_c > k（SpaceClusterK，默认 3.0）→ 簇 c 全部成员标记
4. 记录每卡每时间点的 z_c（被标记簇的 z，其余 0）
...
**设计要点**：
- **基线 = 多数簇**：单卡降频时多数卡兜底，2~7/8 多卡异常也能整簇检出，无 mean/std 稀释
- **方向只用于 tie-break 和单侧判定**：不预设"温度高必异常"，只判"谁偏离多数"
- **scale 自我标定**：来自历史基线，每指标一个值，无需人工调参；唯一旋钮是 k
- **间隙条件防误报**：散布舰队（无主导间隙）不切分 → 不把正常散布边缘当异常
- **边界**：多数本身异常时（如 5/8 热卡）空间保持沉默，由时间维度 + 跨卡关联兜底
