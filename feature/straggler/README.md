...
2. **安装 dyno / dynolog**：wget 下载 msmonitor 8.1.0 包 → 解压 → 从解压目录根取出 `dynolog_*.deb` → 检测系统包管理器（dpkg 原生安装 .deb；rpm 系需 `alien` 转换）→ 安装，使 `dyno` / `dynolog` 直接可从 PATH 调用（已安装则跳过）
...
产物 `./slowNodeDetection`。dyno/dynolog 由第 2 步安装到**系统**（不进仓库，也不再使用 `3rdparty/`）；下载与解压的中间文件在临时目录，退出即清理。改动 Go 代码后只需重跑第 6 步（或直接 `go build`）。
