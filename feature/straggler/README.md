...
**整合模式（推荐）**：由 CATMonitor 底座采集 NPU KPI。启用 CATMonitor 的
`straggler_output`（见 `CATMonitor/configs/catmonitor.yaml`），daemon 会自动写
`straggler_kpi_{date}.jsonl`（按时刻×按卡聚合，保留 15 天），straggler 直接读：
```bash
# CATMonitor 端（启用 straggler_output 后）
catmonitor daemon
# → 产出 {data_dir}/straggler/straggler_kpi_{date}.jsonl
```

**遗留模式**：使用自带 `kpi_collect.sh` 脚本在训练节点上采集 NPU 资源指标，输出为 CSV 文件。
...
整合模式（读 CATMonitor KPI 文件 + 回注 faultsub）：
```bash
...
cd feature/straggler
go mod tidy                      # 首次拉取 modernc.org/sqlite 依赖
CGO_ENABLED=0 go build -o slowNodeDetection .
```

全静态二进制，无 CGo（第二道 Profiler 用纯 Go SQLite 驱动 `modernc.org/sqlite`）。
