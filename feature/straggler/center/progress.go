...
// progressLine is one timestamped entry of a round's live stage log.
type progressLine struct {
	Ts   string `json:"ts"`
	Text string `json:"text"`
}

// progressLog is the in-memory, per-round stage log the business console renders
// terminal-style. It is reset at the start of every trigger round, appended to
// as the round advances, and kept after the round ends so the last round stays
// visible.
...
// finish marks the round as no longer in flight (lines are retained).
...
// progressSnapshot is the GET /center/business/{name}/progress payload.
