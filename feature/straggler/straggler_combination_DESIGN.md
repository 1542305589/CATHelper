...
新增 `FaultType`：`FaultStragglerDetected = "straggler_detected"`（`event.go`）。straggler 回注的事件 `type=straggler_detected`，`detail` 含 `quadrant`、`composite_score`、`anomaly_category`(compute/communication)。订阅者（EEP/运维）按异常指标/空间 score 决定是否触发 Profiler 精查或卡隔离。
...
type FaultEvent struct {  // 与 CATMonitor faultsub 契约一致（JSON）
    ...
    Detail    map[string]string `json:"detail"`      // quadrant/composite_score/anomaly_category
...
{ "type":"straggler_detected", "component":"npu", "npu_id":"3",
  "severity":"critical",
  "detail":{"quadrant":"confirmed_anomaly",
...
|---|---|
| `quadrant=confirmed_anomaly` + `anomaly_category=compute` | 排查硬件/计算（散热、频率、利用率等） |
| `quadrant=confirmed_anomaly` + `anomaly_category=communication` | 排查网络（PFC、重传、带宽等） |
...
| 事件类型 | 新增 straggler_detected | detail 含 quadrant/composite_score/anomaly_category；订阅者按异常指标决策 |
